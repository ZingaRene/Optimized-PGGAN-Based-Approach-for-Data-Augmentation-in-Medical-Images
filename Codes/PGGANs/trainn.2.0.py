...
from progan_modulesss import Generator, Discriminator
...
def imagefolder_loader(path):
    def loader(transform):
        ...
        data_loader = DataLoader(data, shuffle=
...
def train(generator, discriminator, init_step, loader, total_iter=600000):
    ...
    copy('trainn.1.0.py', log_folder+'/train_%s.py'%post_fix)
    copy('progan_moduless.py', log_folder+'/model_%s.py'%post_fix)
    ...
    for i in pbar:
        ...
        os.makedirs(os.path.join(log_folder, 'sample'), exist_ok=True)

        if (i + 1) % 1000 == 0 or i == 0:
         with torch.no_grad():
             num_samples = 5  
        
             for class_idx, class_name in enumerate(['Atelectasis', 'Effusion', 'Pneumonia', 'Cardiomegaly', 'Infiltration']):
                 class_folder = os.path.join(log_folder, 'sample', class_name)
                 os.makedirs(class_folder, exist_ok=True)
            
                 for sample_id in range(num_samples):
                     gen_z = torch.randn(1, input_code_size).to(device)
                     images = g_running(gen_z, step=step, alpha=alpha).data.cpu()

                     utils.save_image(
                         images,
                         f'{class_folder}/{class_name}_sample_{str(i + 1).zfill(6)}_{sample_id}.png',
                         nrow=1,
                         normalize=True
                     )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--total_iter', type=int, default=1000000, help='how many iterations to train in total, the value is in assumption that init step is 1')
    ...
    generator = Generator(in_channel=args.channel, input_code_dim=input_code_size, tanh=args.tanh).to(device)
    
    discriminator = Discriminator(feat_dim=args.channel).to(device)
    g_running = Generator(in_channel=args.channel, input_code_dim=input_code_size, tanh=args.tanh).to(device)
    ...
    if not dataset_path.exists():
        raise FileNotFoundError(f"[ERRO] Caminho especificado não existe: {dataset_path}")

    classes_vazias = []
    for class_dir in dataset_path.iterdir():
        if class_dir.is_dir():
           count = len(list(class_dir.glob("*.png")))
           print(f"[INFO] Classe '{class_dir.name}' contém {count} imagens.")
           if count == 0:
               classes_vazias.append(class_dir.name)

    if classes_vazias:
       raise RuntimeError(f"[ERRO] As seguintes classes estão vazias ou não possuem imagens PNG: {classes_vazias}")
    else:
       print(f"[OK] Todas as classes possuem imagens PNG.")
